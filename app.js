...
function records(){ $('iso-summary').textContent='細菌分離 · '+current.isolation.length+' 筆';$('pcr-summary').textContent='Colony PCR · '+current.pcr.length+' 筆';$('iso-records').innerHTML=current.isolation.map(r=>`<div class="raw-record"><small>細菌分離紀錄表 · 第 ${r.row} 列</small><p><b>${e(C.siteName(r.values[2],current.sites))}</b> · ${e(r.values[15]||'未填結果')} · ${e(r.values[16]||'未填染色結果')}</p><p>培養基：${e(r.values[18]||'—')}　輪值獸醫師：${e(r.values[13]||'—')}</p></div>`).join('')||'<p class="small">沒有分離紀錄。</p>';$('pcr-records').innerHTML=current.pcr.map(r=>`<div class="raw-record"><small>細菌Colony PCR · 第 ${r.row} 列</small><p><b>${e(C.siteName(r.values[2],current.sites))}</b> · ${e(r.values[3])} · ${e(r.values[4])}</p><p>${r.values.slice(6).map((v,i)=>v?e(pcrLabel(i+6))+': '+e(v):'').filter(Boolean).join('　')||'尚無檢測結果'}</p><p>細菌組獸醫師：${e(r.values[5]||'—')}</p></div>`).join('')||'<p class="small">沒有 Colony PCR 紀錄。</p>';}
...
function selectRecord(){selected=current.ast.find(r=>String(r.row)===$('ast-select').value)||null;$('report-controls').hidden=!selected;$('drug-card').hidden=!selected;$('confirmed').checked=false;$('download-status').textContent='';if(!selected)return;const a=selected.values,site=C.siteName(a[2],current.sites);$('meta').innerHTML=[['畜主',a[8]],['收件人',a[9]],['送檢日期',a[4]],['細菌鑑定',a[3]],['分離部位',site],['藥敏來源','第 '+selected.row+' 列']].map(([k,v])=>'<div><small>'+e(k)+'</small><strong>'+e(v||'—')+'</strong></div>').join('');let iso=C.matches(selected,current.isolation,'isolation');opts($('isolation-select'),iso,r=>`第 ${r.row} 列 · ${r.values[16]||'未填分類'} · ${r.values[18]||'未填培養基'}`,'未指定分離紀錄');if(iso.length===1)$('isolation-select').value=String(iso[0].row);let pcr=C.matches(selected,current.pcr,'pcr');opts($('pcr-select'),pcr,r=>`第 ${r.row} 列 · ${r.values[3]} · ${r.values[4]}`,'不帶入毒力基因結果');$('pcr-select').disabled=!/Escherichia coli/i.test(a[3]||'');$('site-text').value=site;$('report-date').value=new Date().toLocaleDateString('en-CA',{timeZone:'Asia/Taipei'});$('match-note').textContent=`同部位且同菌名的分離紀錄 ${iso.length} 筆；同部位 PCR ${pcr.length} 筆。${site===C.clean(a[2])&&/^[A-Za-z() ,]+$/.test(site)?'此部位在縮寫對照表無中文對應，請人工確認。':''}E. coli 毒力結果須手動選擇並核對培養基，不能只依編號帶入。`;$('kind').value=C.gram(iso.length===1?iso[0]:null);selectDutyVet();refresh();}
...
function refresh(){if(!selected)return;$('confirmed').checked=false;const vet=$('vet-select').value,stamp=ASSETS.stamps[vet],professor=ASSETS.stamps['林章豪'];$('stamp-status').classList.toggle('warning',!stamp||!professor);$('stamp-status').innerHTML=(stamp?`<span>檢驗人員：${e(vet)}<br>已找到對應印章</span><img alt="${e(vet)}印章" src="data:image/${stamp.ext==='jpeg'?'jpeg':'png'};base64,${stamp.data}">`:`檢驗人員：${e(vet||'未指定')} · 未找到對應印章，報告將標示「待補章」。`)+(professor?`<span>副教授：林章豪<br>已找到對應印章</span><img alt="林章豪印章" src="data:image/${professor.ext==='jpeg'?'jpeg':'png'};base64,${professor.data}">`:'<span>副教授：林章豪 · 未找到印章，報告將標示「待補章」。</span>');const kind=$('kind').value;$('drugs').innerHTML=kind?C.drugRows(selected,kind).map(d=>`<tr><td>${e(d.name)}<small>${e(d.code)}</small></td><td>${e(d.dose)}</td><td>${e(d.mm||'—')}</td><td><span class="badge ${e(d.state)}" title="${e(d.label)}">${e(d.value)}</span>${d.state==='error'?'<small>'+e(d.label)+'</small>':''}</td></tr>`).join(''):'<tr><td colspan="4">請選擇陽性菌或陰性菌範本後，查看對應藥物。</td></tr>';enable();}
...
function model(){if($('word').disabled)throw Error('請先完成報告欄位並核對資料。');return LabReport.model({record:selected,kind:$('kind').value,pcr:current.pcr.find(r=>String(r.row)===$('pcr-select').value),date:$('report-date').value,site:$('site-text').value.trim(),vet:$('vet-select').value});}
...
$('ast-select').addEventListener('change',selectRecord);$('isolation-select').addEventListener('change',()=>{let iso=current.isolation.find(r=>String(r.row)===$('isolation-select').value);$('kind').value=C.gram(iso);selectDutyVet();refresh();});for(const id of ['kind','pcr-select','report-date','site-text','vet-select'])$(id).addEventListener('change',refresh);$('confirmed').addEventListener('change',enable);
$('word').addEventListener('click',async()=>{try{let m=model(),name=LabReport.filenameBase(m);$('download-status').textContent='正在套用 Word 範本…';$('word').disabled=true;const blob=await LabReport.docx(m);save(blob,name+'.docx');$('download-status').textContent='Word 已產生。';}catch(err){$('download-status').textContent='無法產生：'+err.message;}finally{enable();}});
$('pdf').addEventListener('click',()=>{try{let m=model(),html=LabReport.printHtml(m),w=window.open('','_blank');if(!w)throw Error('請允許此網站開啟報告視窗。');w.document.open();w.document.write(html);w.document.close();}catch(err){$('download-status').textContent=err.message;}});
showSource();
