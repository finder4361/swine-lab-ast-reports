...
function find(data,id){const n=norm(id);if(!/^\d{2,3}-\d+$/.test(n))throw Error('請輸入完整免疫室編號，例如 115-2651。');return {id:n,isolation:indexed(data.isolation,2).filter(x=>norm(x.values[1])===n),pcr:indexed(data.pcr,3).filter(x=>norm(x.values[1])===n),ast:indexed(data.ast,3).filter(x=>norm(x.values[1])===n)};}
...
const api={clean,norm,drugs,genes,indexed,find,matches,interpretation,drugRows,gram,pcrText};
